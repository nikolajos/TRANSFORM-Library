...
model StraightPipe
  parameter SI.Length dimension = 4*crossArea/perimeter "Characteristic dimension (e.g., hydraulic diameter)"
  ...
  parameter SI.Length perimeter=4*crossArea/dimension "Wetted perimeters"
   annotation (Dialog(group="Inputs"));
  parameter SI.Length length = 1.0 "Pipe length"
    annotation (Dialog(group="Inputs"));
  parameter SI.Height roughness = 2.5e-5 "Average heights of surface asperities"
    annotation (Dialog(group="Inputs"));
  parameter SI.Area surfaceArea[nSurfaces] = {if i == 1 then perimeter*length else 0 for i in 1:nSurfaces} "Area per transfer surface"
   annotation (Dialog(group="Inputs"));
  // Static head
  parameter SI.Angle angle=0.0 "Vertical angle from the horizontal (-pi/2 < x <= pi/2)"
    annotation (Dialog(group="Inputs Elevation"));
  parameter SI.Length dheight= length*sin(angle)
