...
model GenericPipe
  ...
  input SI.Area surfaceAreas[nV,nSurfaces]={{if j == 1 then perimeters[i]*dlengths[i] else 0 for j in 1:nSurfaces} for i in 1:nV}
    ...
    annotation (Dialog(group="Inputs"));
  // Elevation
  input SI.Angle[nV] angles(min=fill(-Modelica.Constants.pi/2-0.01, nV),max=fill(Modelica.Constants.pi/2+0.01, nV))=fill(0,nV) "Vertical angle from the horizontal (-pi/2 <= x <= pi/2)"
